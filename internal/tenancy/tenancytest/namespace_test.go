...
func TestListNamespace_Success(t *testing.T) {
	...
	listRsp, err := cl.POCList(context.Background(), &pbresource.POCListRequest{Request: &pbresource.POCListRequest_FilterByType{FilterByType: &pbresource.POCListRequest_ListByTypeRequest{Type: pbtenancy.NamespaceType, Tenancy: resource.DefaultPartitionedTenancy()}}})
	require.NoError(t, err)
