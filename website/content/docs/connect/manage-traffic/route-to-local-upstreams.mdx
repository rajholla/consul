...
!> **Warning**: Locality-aware routing is an advanced feature that may adversely impact service capacity if used incorrectly. By default, this feature routes 100% of traffic to the most local set of service instances, and failover only occurs when there are no healthy instances available in the most local set. You should only follow these instructions when each set of upstream instances (within the same zone, within the same region, and non-local) have enough capacity to handle requests from downstream service instances in their respective zone(s).

-> **Note**: It is possible to adjust the load balancing and failover behavior for this feature globally or per-service via the [Property Override Envoy extension](/consul/docs/connect/proxies/envoy-extensions/usage/property-override) by modifying [`overprovisioning_factor`](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/endpoint/v3/endpoint.proto#config-endpoint-v3-clusterloadassignment) and [outlier detection](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/cluster/v3/outlier_detection.proto#config-cluster-v3-outlierdetection) configuration. By default, Consul sets `overprovisioning_factor` to `100000` (enforcing "total" failover) and `max_ejection_percent` to `100`. Please familiarize with Envoy documentation on these fields before attempting to modify them.
...
For Kubernetes-orchestrated networks using `consul-k8s`, Consul automatically populates geographic information about service instances using the `topology.kubernetes.io/region` and `topology.kubernetes.io/zone` labels from the Kubernetes nodes. Similarly, when deploying on AWS ECS using `consul-ecs`, the `AWS_REGION` environment variable and `AvailabilityZone` attribute of the ECS task meta are used. As a result, you do not need to specify the regions and zones on these platforms unless you are implementing a custom deployment.
...
### Explicit locality configuration on Kubernetes test clusters

If you are testing this feature with a local Kubernetes cluster (or in an environment where `topology.kubernetes.io` labels are not set), it is possible to explicitly provide each node's locality using `kubectl label`:

```shell-session
kubectl label node $K8S_NODE topology.kubernetes.io/region="us-east-1" topology.kubernetes.io/zone="us-east-1a"
```

After setting these values, you should see subsequent service and proxy registrations in your cluster inherit the values from their local Kubernetes node.

## Observing routing changes

This feature relies on configuring Envoy's built-in [`overprovisioning_factor`](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/endpoint/v3/endpoint.proto#config-endpoint-v3-clusterloadassignment) and [outlier detection](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/cluster/v3/outlier_detection.proto#config-cluster-v3-outlierdetection) configuration to enforce failover behavior. Because Envoy does not provide granular metrics specific to failover or endpoint traffic within a cluster, the effects of enabling this feature are best observed using external observability tools that expose network traffic within your environment. _The immediate effect of enabling the feature should be the local routing of traffic from each downstream service instance to the most-local set of healthy upstream instances available._

To verify locality-aware routing and failover configuration is in place, you can inspect Envoy's xDS config dump for a downstream proxy. Refer to the [consul-k8s CLI docs](https://developer.hashicorp.com/consul/docs/k8s/k8s-cli#proxy-read) for details on how to obtain this on Kubernetes; for other workloads, use the Envoy [admin interface](https://www.envoyproxy.io/docs/envoy/latest/operations/admin) and ensure that you [include EDS](https://www.envoyproxy.io/docs/envoy/latest/operations/admin#get--config_dump?include_eds). Inspect the [priority](https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/load_balancing/priority#arch-overview-load-balancing-priority-levels) on each set of endpoints under the upstream `ClusterLoadAssignment` in the `EndpointsConfigDump`. Alternatively, the same priorities should be visibile within the output of the [`/clusters?format=json`](https://www.envoyproxy.io/docs/envoy/latest/operations/admin#get--clusters?format=json) admin endpoint.
...
To force a failover for testing purposes, scale the upstream service instances in the downstream's local zone (or region, if no local zone instances are avaiable) to zero.

<Tip>
Keep in mind the following:
  - Priority for failover is in ascending order, starting with `0`. Priority `0` (the "highest" priority) will not be explicitly visible in xDS output, since it is the default value for that field.
  - Once Envoy failover configuration is in place, the specific timing of failover is determined by the downstream Envoy proxy, not Consul. Consul health status may not correspond 1-1 with Envoy's failover behavior, which is also dependent on outlier detection.
</Tip>

## Troubleshooting

If you do not see the expected priorities, verify that your locality configuration is in place and that you have enabled `PrioritizeByLocality` in your proxy defaults or service resolver. If `PrioritizeByLocality` is enabled but the local proxy lacks locality configuration, Consul will emit a warning log to indicate that the policy could not be applied:
